...
class AutoSpacingCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        folded_regions_content = [
            self.view.substr(r) for r in self.view.folded_regions()
        ]
    ...
    def run_script_on_file(self, data):
        try:
            ...
            output = PluginUtils.automate(viewText)
...
class PluginUtils:
    ...
    def automate(text):
        new_text = text
        new_text = PluginUtils.separator(new_text)
        print("=> separator ")
        new_text = PluginUtils.spacing(new_text)
        print("=> spacing ")
